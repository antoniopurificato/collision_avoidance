...
void laser_callback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
    ...
    float actual_distance, current_distance;
    Eigen::Vector2f actual_position,current_position;
    
    actual_position(0) = cloud.points[540].x;
    actual_position(1) = cloud.points[540].y;

    /*I compute the actual distance between the laser scan of my robot and the closest obstacle*/
    actual_position = laser_tf * actual_position;
    actual_distance = sqrt(pow(actual_position(0),2)+pow(actual_position(1),2));
    ...
    for(auto& point: cloud.points){         
        ...
        current_distance = sqrt(pow(current_position(0),2) + pow(current_position(1),2));
        ...
        /*If i'm closer to the obstacle wrt. the previous position i update the initial position with the current position*/
        if(current_distance < actual_distance){
    ...
    if(actual_distance < 0.2){
        float magnitude = (1.0 / actual_distance);
